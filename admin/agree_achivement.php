...
if(isset($_GET['delete'])){
  $sno = $_GET['delete'];
  $delete = true;
  $sql = "DELETE FROM `agree_achivement` WHERE `sno` = $sno";
...
if($_SERVER["REQUEST_METHOD"]=='POST'){
  ...
  if (isset( $_POST['snoEdit'])){
   ...
   // $title5=mysqli_real_escape_string($db,$_POST['title5']);
    ...
    $sql="UPDATE `agree_achivement` SET `name_of_work`='$title',`contract_id`='$title1',`name_contractor`='$title2',`Contract Amount`='$title3',`Dates of Agreement`='$title4',`Dates of completion`='$title6',`Due Dates of Completion`='$title7',`Time Extended`='$title8',`Time Elapsed`='$title9',`financial_Amount`='$title10',`financial_Percent`='$title11',`financial_dates`='$title12',`Physical_Percent`='$title13',`Physical_Dates`='$title14',`Progress Statuss`='$title15',`Remarks`='$title16' WHERE `agree_achivement`.`sno` = $news_id;";
 ...
 // $title5=mysqli_real_escape_string($db,$_POST['title5']);
  ...
  $sql="INSERT INTO `agree_achivement` (`name_of_work`, `contract_id`, `name_contractor`, `Contract Amount`, `Dates of Agreement`, `Dates of completion`, `Due Dates of Completion`, `Time Extended`, `Time Elapsed`, `financial_Amount`, `financial_Percent`, `financial_dates`, `Physical_Percent`, `Physical_Dates`, `Progress Statuss`, `Remarks`) VALUES ('$title', '$title1', '$title2', '$title3', '$title4', '$title6', '$title7', '$title8', '$title9', '$title10', '$title11', '$title12', '$title13', '$title14', '$title15', '$title16')";
    ...
    <div class="pagetitle">
      ...
      <nav>
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item active">Agreement Achievement</li>
        </ol>
      </nav>
    </div><!-- End Page Title -->
    ...
    <table class="table" id="myTable">
      <thead>
        <tr>
          ...
          <th width="5%" scope="col">Name of work</th>
          <th width="" scope="col">Contract ID	</th>
          <th width="" scope="col">Name of Contractor</th>
          <th width="" scope="col">Contract Amount	</th>
          <th width="" scope="col">Date of Agreement	</th>
          <th width="" scope="col">Date of completion as per Agreement</th>
          <th width="" scope="col">Due Date of Completion</th>
          <th width="" scope="col">Time Extended based on Original schedule	</th>
          <th width="" scope="col">Time Elapsed based on revised schedule</th>
          <th width="" scope="col"> Financial Amount</th>
          <th width="" scope="col">	Finacial Percent</th>
          <th width="" scope="col">Financial Dates	</th>
          <th width="" scope="col">Physical Percent</th>
          <th width="" scope="col">Physical Dates</th>
          <th width="" scope="col">Progress Status</th>
      ...
      <tbody>
        <?php 
          ...
          while($row = mysqli_fetch_assoc($result)){
            ...
            <td>". $row['name_contractor'] . "</td>
            <td>". $row['Contract Amount'] . "</td>
            <td>". $row['Dates of Agreement'] . "</td>
            <td>". $row['Dates of completion'] . "</td>
            <td>". $row['Due Dates of Completion'] . "</td>
            <td>". $row['Time Extended'] . "</td>
            <td>". $row['Time Elapsed'] . "</td>
            <td>". $row['financial_Amount'] . "</td>
            <td>". $row['financial_Percent'] . "</td>
            <td>". $row['financial_dates'] . "</td>
            <td>". $row['Physical_Percent'] . "</td>
            <td>". $row['Physical_Dates'] . "</td>
            <td>". $row['Progress Statuss'] . "</td>
            <td>". $row['Remarks'] . "</td>
    ...
    Array.from(deletes).forEach((element) => {
      element.addEventListener("click", (e) => {
        ...
        if (confirm("Are you sure you want to delete this data!")) {
          console.log("yes");
          window.location = `/admin/agree_achivement.php?delete=${sno}`;
