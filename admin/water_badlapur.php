...
if(isset($_GET['delete'])){
  $sno = $_GET['delete'];
  $delete = true;
  $sql = "DELETE FROM `kulopani_prag_table` WHERE `table_id` = $sno";
...
if($_SERVER["REQUEST_METHOD"]=='POST'){
  ...
  if (isset( $_POST['snoEdit'])){
    ...
    if($fileerror==0){
      ...
      $sql = "SELECT * FROM `kulopani_prag_table` WHERE `card_id`=$news_id";
      $result = mysqli_query($db, $sql);
    ...
    $sql="UPDATE `kulopani_prag_table` SET `fullname`='$title1',`address`='$title2',`designation`='$title3', `contact`='$title4',`profile` = '$destfile',`unique_id`='$title5' WHERE `kulopani_prag_table`.`table_id` = $news_id;";
  ...
  $sql="INSERT INTO `kulopani_prag_table` (`fullname`, `address`, `designation`,`contact`,`profile`,`unique_id`) VALUES ('$title1', '$title2', '$title3','$title4','$destfile','$title5')";
...
?>
    ...
    <div class="pagetitle">
      ...
      <nav>
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item active">Water User Association / Badlapur</li>
        </ol>
      </nav>
    </div><!-- End Page Title -->
  ...
  <div class="container my-4">
    ...
    <table class="table" id="myTable">
      ...
      <tbody>
        <?php 
          $sql = "SELECT * FROM `kulopani_prag_table`";
          $result = mysqli_query($db, $sql);
          $sno = 0;
          while($row = mysqli_fetch_assoc($result)){
            $sno = $sno + 1;
  ...
  edit = document.getElementsByClassName('edit1');
    ...
    Array.from(deletes).forEach((element) => {
      element.addEventListener("click", (e) => {
        ...
        if (confirm("Are you sure you want to delete this data!")) {
          console.log("yes");
          window.location = `/admin/water_badlapur.php?delete=${sno}`;
    ...
    Array.from(delete1).forEach((element) => {
      element.addEventListener("click", (e) => {
        ...
        if (confirm("Are you sure you want to delete this data!")) {
          console.log("yes");
          window.location = `/admin/action1.php?delete1=${sno}`;
          // TODO: Create a form and use post request to submit a form
        }
        else {
          console.log("no");
