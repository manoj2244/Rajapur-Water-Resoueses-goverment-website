...
if(isset($_GET['delete'])){
  $sno = $_GET['delete'];
  $delete = true;
  $sql = "DELETE FROM `organization` WHERE `id` = $sno";
...
if($_SERVER["REQUEST_METHOD"]=='POST'){
  ...
  if (isset( $_POST['snoEdit'])){
    ...
    if($fileerror==0){
        $destfile='../About-US_File/gov_image/' .$filename;
        move_uploaded_file($filepath, $destfile);
    }
    else{
      $sql = "SELECT * FROM `organization` WHERE `image_no`=$image_no";
      $result = mysqli_query($db, $sql);
    ...
    $sql="UPDATE `organization` SET  `photo` = '$destfile' WHERE `organization`.`id` = $image_no;";
  ...
  $sql="INSERT INTO `organization` (`photo`) VALUES ('$destfile')";
    ...
    Array.from(deletes).forEach((element) => {
      element.addEventListener("click", (e) => {
        ...
        if (confirm("Are you sure you want to delete this photos!")) {
          console.log("yes");
          window.location = `/admin/organization_image.php?delete=${sno}`;
