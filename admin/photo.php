...
if(isset($_GET['delete'])){
  $sno = $_GET['delete'];
  $delete = true;
  $sql = "DELETE FROM `gallery` WHERE `image_no` = $sno";
...
if($_SERVER["REQUEST_METHOD"]=='POST'){
  ...
  if (isset( $_POST['snoEdit'])){
    ...
    if($fileerror==0){
        $destfile='../gallery_file/gallery_images/' .$filename;
        move_uploaded_file($filepath, $destfile);
    }
    else{
      $sql = "SELECT * FROM `gallery` WHERE `image_no`=$image_no";
      $result = mysqli_query($db, $sql);
    ...
    $sql="UPDATE `gallery` SET  `images_name` = '$destfile',`image_title` = '$title' WHERE `gallery`.`image_no` = $image_no;";
  ...
  $sql="INSERT INTO `gallery` (`images_name`,`image_title`) VALUES ('$destfile','$title')";
      ...
      <tbody>
        <?php 
          ...
          while($row = mysqli_fetch_assoc($result)){
            ...
            echo "<tr>
            <th scope='row'>". $sno  . "</th>
            <td>". $row['images_name'] . "</td>
            <td> <img src='".$row['images_name']."' style='width:150px;height:150px;object-fit:cover;'></td>
            <td>". $row['image_title'] ."</td>
            
            <td> <button class='edit btn btn-sm btn-primary'id=".$row['image_no'].">Edit</button> <button class='delete btn btn-sm btn-primary' id=d".$row['image_no'].">Delete</button></td>
          </tr>";
        } 
