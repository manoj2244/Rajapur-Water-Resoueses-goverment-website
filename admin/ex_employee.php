...
if(isset($_GET['delete'])){
  $sno = $_GET['delete'];
  $delete = true;
  $sql = "DELETE FROM `ex_employee` WHERE `card_id` = $sno";
...
if($_SERVER["REQUEST_METHOD"]=='POST'){
  ...
  if (isset( $_POST['snoEdit'])){
    ...
    if($fileerror==0){
      ...
      $sql = "SELECT * FROM `ex_employee` WHERE `card_id`=$news_id";
      $result = mysqli_query($db, $sql);
    ...
    $sql="UPDATE `ex_employee` SET `name` = '$title',`profession`='$title1',`sereni`='$title2',`phone`='$title3', `kaifiyat`='$title4',`card_image` = '$destfile' WHERE `ex_employee`.`card_id` = $news_id;";
  ...
  $sql="INSERT INTO `ex_employee` (`name`, `profession`, `sereni`, `phone`,`kaifiyat`,`card_image`) VALUES ('$title', '$title1', '$title2', '$title3','$title4','$destfile')";
...
?>
  ...
  ?>
    ...
    <div class="pagetitle">
      ...
      <nav>
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item active">ex_employee</li>
        </ol>
      </nav>
    </div><!-- End Page Title -->
    ...
    <h2>Add a Ex_Employee Details</h2>
    <form action="" method="POST" enctype="multipart/form-data">
      <div class="form-group">
        <label for="title">नाम</label>
    ...
    Array.from(deletes).forEach((element) => {
      element.addEventListener("click", (e) => {
        ...
        if (confirm("Are you sure you want to delete this data!")) {
          console.log("yes");
          window.location = `/admin/ex_employee.php?delete=${sno}`;
