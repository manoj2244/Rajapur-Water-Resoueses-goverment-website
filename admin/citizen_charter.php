...
if(isset($_GET['delete'])){
  $sno = $_GET['delete'];
  $delete = true;
  $sql = "DELETE FROM `citizen_charter` WHERE `id` = $sno";
...
if($_SERVER["REQUEST_METHOD"]=='POST'){
  ...
  if (isset( $_POST['snoEdit'])){
    ...
    if($fileerror==0){
      ...
      $sql = "SELECT * FROM `citizen_charter` WHERE `image_no`=$image_no";
      $result = mysqli_query($db, $sql);
    ...
    $sql="UPDATE `citizen_charter` SET  `image` = '$destfile' WHERE `citizen_charter`.`id` = $image_no;";
  ...
  $sql="INSERT INTO `citizen_charter` (`image`) VALUES ('$destfile')";
    ...
    <div class="pagetitle">
      ...
      <nav>
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item active">citizen_charter</li>
        </ol>
      </nav>
    </div><!-- End Page Title -->
    ...
    Array.from(deletes).forEach((element) => {
      element.addEventListener("click", (e) => {
        ...
        if (confirm("Are you sure you want to delete this photos!")) {
          console.log("yes");
          window.location = `/admin/citizen_charter.php?delete=${sno}`;
