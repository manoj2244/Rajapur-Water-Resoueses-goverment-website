...
if(isset($_GET['delete'])){
  $sno = $_GET['delete'];
  $delete = true;
  $sql = "DELETE FROM `main_intro` WHERE `id` = $sno";
...
if($_SERVER["REQUEST_METHOD"]=='POST'){
  ...
  if (isset( $_POST['snoEdit'])){
    ...
    if($fileerror==0){
      ...
      $sql = "SELECT * FROM `main_intro` WHERE `id`=$news_id";
      $result = mysqli_query($db, $sql);
    ...
    $sql="UPDATE `main_intro` SET `introduction` = '$title', `photo` = '$destfile' WHERE `main_intro`.`id` = $news_id;";
  ...
  $sql="INSERT INTO `main_intro` (`introduction`, `photo`) VALUES ('$title', '$destfile')";
...
<body>
   ...
   <!-- Basic Modal -->
              ...
              <textarea class="form-control" id="title" name="title" rows="3" value=""></textarea>
                    ...
                    <button type="submit" class="btn btn-primary my-2">update intro</button>
                  </form>
                    </div>
...
?>
  ...
  ?>
    ...
    <div class="pagetitle">
      ...
      <nav>
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item active">main_intro</li>
        </ol>
      </nav>
    </div><!-- End Page Title -->
    ...
    <h2>Add  main introduction and project image</h2>
    <form action="" method="POST" enctype="multipart/form-data">
    <div class="form-group">
              <label for="desc">Introduction</label>
    ...
    <table class="table" id="myTable">
      <thead>
        <tr>
          ...
          <th width="50%" scope="col">Introduction</th>
      ...
      <tbody>
        <?php 
          ...
          while($row = mysqli_fetch_assoc($result)){
            ...
            <td> <img src='".$row['photo']."' style='width:150px;height:150px;object-fit:cover;'></td>
    ...
    Array.from(deletes).forEach((element) => {
      element.addEventListener("click", (e) => {
        ...
        if (confirm("Are you sure you want to delete this data!")) {
          console.log("yes");
          window.location = `/admin/home_introduction.php?delete=${sno}`;
