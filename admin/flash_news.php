...
if(isset($_GET['delete'])){
  $sno = $_GET['delete'];
  $delete = true;
  $sql = "DELETE FROM `flash_news` WHERE `News_id` = $sno";
...
if($_SERVER["REQUEST_METHOD"]=='POST'){
  ...
  if (isset( $_POST['snoEdit'])){
    ...
    if($fileerror==0){
        $destfile='../Information_And_News/news_file/' .$filename;
    ...
    $sql="UPDATE `flash_news` SET `News_title` = '$title',`date`='$title1', `pdf_file` = '$destfile' WHERE `flash_news`.`News_id` = $news_id";
  ...
  $sql="INSERT INTO `flash_news` (`News_title`, `posted_at`, `pdf_file`, `date`, `view`) VALUES ('$title', current_timestamp(), '$destfile', '$title1', '0')";
              ...
              <div class="modal fade" id="editModal" tabindex="-1">
                <div class="modal-dialog">
                  <div class="modal-content">
                    ...
                    <button type="submit" class="btn btn-primary my-2">Add News</button>
                  </form>
                    </div>
...
?>
  ...
  if($insert==1){
    ...
    Your news has been inserted successfully
    <button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>
  </div>";
  }
  if($insert==0){
   echo" <div class='alert alert-danger alert-dismissible fade show' role='alert'>
    Your news is not added sucessfully!
    <button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>
  </div>";
  }
  if($update==1){
  ...
  if($update==0){
   echo" <div class='alert alert-danger alert-dismissible fade show' role='alert'>
    Your news is not updated sucessfully!
    <button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>
  </div>";
  }
    ...
    <div class="pagetitle">
      ...
      <nav>
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item active">flash_news</li>
        </ol>
      </nav>
    </div><!-- End Page Title -->
      ...
      <tbody>
        <?php 
          ...
          while($row = mysqli_fetch_assoc($result)){
            ...
            <td>".substr($row['pdf_file'],-14) . "</td>
            <td> <button class='edit btn btn-sm btn-primary'id=".$row['News_id'].">Edit</button> <button class='delete btn btn-sm btn-primary' id=d".$row['News_id'].">Delete</button></td>
          </tr>";
        } 
          ?>
    ...
    Array.from(deletes).forEach((element) => {
      element.addEventListener("click", (e) => {
        ...
        if (confirm("Are you sure you want to delete this news!")) {
          console.log("yes");
          window.location = `/admin/flash_news.php?delete=${sno}`;
